...
async function getContent(wxss, filePath, cwd) {
  ...
  if (wxss) {
    ...
    for (const item of currentImportList) {
      ...
      wxss = wxss.replace(
        item.code,
        `/* *updated for miniprogram-custom-component* ${item.code} */`
      );
...
module.exports = {
  start() {
    return through.obj(function(file, enc, cb) {
      if (file.isBuffer()) {
        getContent(file.contents.toString('utf8'), file.path, file.cwd)
          .then(res => {
    ...
    return through.obj(function(file, enc, cb) {
      if (file.isBuffer) {
        const reg = /\/\*\s\*updated for miniprogram-custom-component\*\s(@import\s+(?:(?:"([^"]+)")|(?:'([^"]+)'));)\s\*\//gi;
        const wxss = file.contents
          .toString('utf8')
          .replace(reg, (all, $1) => $1);
