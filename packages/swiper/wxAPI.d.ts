...
type WXMergedAPI = WXNetAPI & WXMediaAPI & WXStorageAPI & WXLocationAPI & WXDeviceAPI & WXUIAPI & WXOpenAPI & WXAny;
...
interface WXChooseImageObj {
	...
	sizeType?: string[];
	sourceType?: string[];
	success: (res: WXTempFileObj) => void;
	fail: WXCommonCallback;
	complete: WXCommonCallback;
}
...
interface WXGetSaveFileObj {
	success?: (res: { errMsg: string, fileList: Array<{ filePath: string, createTime: number, size: number, [propName: string]: any }> }) => void;
	fail?: WXCommonCallback;
	complete?: WXCommonCallback;
}
...
interface WXLocationAPI {
	...
	chooseAddress(obj: { success?: (res: IWxAddress) => void, fail?: WXCommonCallback, complete?: WXCommonCallback })
}
...
interface WXContext {
	...
	save();
	draw(reserve?: boolean, callback?: Function);
	restore();
...
interface WXOpenAPI {
	...
	getUserInfo(obj: { success?: (res: userInfo) => void, fail?: WXCommonCallback, complete?: WXCommonCallback });
...
interface WXImageLoad extends WXEventBasic {
	detail: {
		width: string;
		height: string;
	}
...
declare let Component: IComponent;
